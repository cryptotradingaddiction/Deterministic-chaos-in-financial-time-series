...
----------------------------------------------------------------
This script downloads complete hourly OHLCV data for multiple cryptocurrency
pairs from Bitstamp (or any CCXT-supported exchange). It performs rigorous
checks to ensure no single hour is missing within the downloaded range and
that the data realistically covers the requested date span.
"""
...
def download_hourly_data(symbol: str,
    ...
    print(f"[OK] Data is complete, contiguous, and meets boundary requirements. {len(df)} hours.")
    return df
