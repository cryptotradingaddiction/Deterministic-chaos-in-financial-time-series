...
window_size = 720
tolerance = 1.0
apply_cut_to_source_file = True
create_backup_before_cut = True


def cut_dataset_from_optimal_start(df, optimal_start):
    """
    Keep only rows from the first timestamp where liquidity is acceptable.
    """
    return df[df.index >= optimal_start].copy()


def save_cut_dataset(cut_df, source_file_path, symbol, make_backup=True):
    """
    Save the cut dataset back to the original CSV, optionally creating a backup.
    """
    if make_backup:
    ...
    output_df.to_csv(source_file_path, index=False)
    print(f"{symbol}: source file cut and saved -> {source_file_path}")


def save_cut_dat_file(source_dat_path, rows_to_skip, symbol, make_backup=True):
    """
    Cut the matching .dat file by removing the same number of leading rows.
    """
    ...
    with open(source_dat_path, "w", encoding="utf-8") as f:
        f.writelines(cut_lines)

    print(f"{symbol}: .dat file cut and saved -> {source_dat_path}")
...
for filename in files:
    ...
    valid_data = df[df['zero_pct'] < tolerance]

    if not valid_data.empty:
        optimal_start = valid_data.index[0]
        print(f"{symbol}: optimal start date = {optimal_start.date()} (zero_pct < {tolerance}%)")
        if apply_cut_to_source_file:
            rows_to_skip = int((df.index < optimal_start).sum())
            cut_df = cut_dataset_from_optimal_start(df, optimal_start)
            save_cut_dataset(
                cut_df=cut_df,
