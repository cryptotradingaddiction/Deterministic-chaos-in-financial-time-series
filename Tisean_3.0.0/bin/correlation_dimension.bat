...
> "%TAKENS_AGG_FILE%" echo symbol,run,tau,W,takens_file,ellner_m3,plateau_points
...
REM Keep the standalone diagnostic plots on the same radius interval as
REM hypothesis.py: the book-oriented Takens/Ellner range is 0.25*sigma..0.5*sigma.
set "EPS_MIN="
set "EPS_MAX="
for /f "usebackq tokens=1,2" %%A in (`"%PYTHON_EXE%" %PYTHON_ARGS% -c "import numpy as np, sys; x=np.loadtxt(sys.argv[1]); s=float(np.std(x, ddof=1)); print(f'{0.25*s:.12g} {0.5*s:.12g}')" "!DATA_FILE!"`) do (
    set "EPS_MIN=%%A"
    set "EPS_MAX=%%B"
)
echo   Radius range: r_min=!EPS_MIN! r_max=!EPS_MAX! (0.25*sigma .. 0.5*sigma)

echo   [1/3] d2: correlation sums for Takens/Ellner (+ diagnostic local slopes)...
"%TISEAN%\d2.exe" -d!TAU_DELAY! -M%EMBED% -t!THEILER_W! -#100 -N0 -r!EPS_MIN! -R!EPS_MAX! -o "!OUT_DIR!\!BASE!" "!DATA_FILE!"
if errorlevel 1 exit /b 1
...
for /f "usebackq tokens=1,2,3 delims=," %%A in ("!TAKENS_VALUE_TMP!") do (
    >> "%TAKENS_AGG_FILE%" echo !BASE!,!RUN_ID!,!TAU_DELAY!,!THEILER_W!,!OUT_DIR!\!BASE!_takens.dat,%%B,%%C
)
...
echo   [3/3] plot: log C, diagnostic local d_2, Takens, and Ellner estimates vs ln r ...
if /i "%HAS_GNUPLOT%"=="true" (
    ...
    "%GNUPLOT_EXE%" -e "set terminal pngcairo size 1400,900 enhanced font 'Arial,12'; set output '!OUT_DIR!\!BASE!_takens_all_m.png'; set grid; set xlabel 'ln r'; set ylabel 'd_2^T'; set title '!BASE! Takens-Theiler estimate d_2^T (tau=!TAU_DELAY!, W=!THEILER_W!)!PLOT_SUFFIX!'; set key right bottom font 'Arial,7' vertical maxrows 30; plot for [idx=0:*] '!OUT_DIR!\!BASE!_takens.dat' index idx using (log($1)):2 with lines lw 1 title sprintf('m=%%d', idx+1)" >> "!OUT_DIR!\gnuplot.log" 2>&1
    "%PYTHON_EXE%" %PYTHON_ARGS% "%PRINT_RESULTS%" file "!OUT_DIR!\!BASE!_takens_all_m.png"
    "%GNUPLOT_EXE%" -e "set terminal pngcairo size 1400,900 enhanced font 'Arial,12'; set output '!OUT_DIR!\!BASE!_ellner_all_m.png'; set grid; set xlabel 'ln r'; set ylabel 'd_2^E'; set title '!BASE! Ellner interval estimate d_2^E (tau=!TAU_DELAY!, W=!THEILER_W!)!PLOT_SUFFIX!'; set key right bottom font 'Arial,7' vertical maxrows 30; plot for [idx=0:*] '!OUT_DIR!\!BASE!_ellner.dat' index idx using (log($1)):2 with lines lw 2 title sprintf('m=%%d', idx+1)" >> "!OUT_DIR!\gnuplot.log" 2>&1
    "%PYTHON_EXE%" %PYTHON_ARGS% "%PRINT_RESULTS%" file "!OUT_DIR!\!BASE!_ellner_all_m.png"
)
echo(
exit /b 0
